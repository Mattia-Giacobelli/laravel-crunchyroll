...
@section('main')
    <div class="container mt-5">
        ...
        <form action="{{ route('user.anime.update', $anime) }}" method="POST" enctype="multipart/form-data"
            data-bs-theme="dark">
                ...
                <select class="form-select" id="animation_studio_id" name="animation_studio_id">
                    ...
                    @foreach ($studios as $studio)
                        <option value="{{ $studio->id }}"
                            {{ $anime->animation_studio_id == $studio->id ? 'selected' : '' }}>
                            {{ $studio->name }}
                        </option>
                    @endforeach
            ...
            @foreach ($genres as $genre)
                <div class="form-check form-check-inline mb-3">
                    <input name="genres[]" value="{{ $genre->id }}" class="form-check-input" type="checkbox"
                        id="genre{{ $genre->id }}" {{ $anime->genres->contains($genre->id) ? 'checked' : '' }}>
            ...
            @foreach ($dubs as $dub)
                <div class="form-check form-check-inline mb-3">
                    <input name="dubs[]" value="{{ $dub->id }}" class="form-check-input" type="checkbox"
                        id="dub{{ $dub->id }}" {{ $anime->dubs->contains($dub->id) ? 'checked' : '' }}>
            ...
            @foreach ($subs as $sub)
                <div class="form-check form-check-inline mb-3">
                    <input name="subs[]" value="{{ $sub->id }}" class="form-check-input" type="checkbox"
                        id="sub{{ $sub->id }}" {{ $anime->subs->contains($sub->id) ? 'checked' : '' }}>
